...
def model15(input_shape):
    ...
    # Block0
    X = Conv2D(filters=32, kernel_size=(3, 3), strides=1, padding='same', name='conv2D_0')(X_input)
    ...
    # Block1
    X = Conv2D(filters=64, kernel_size=(3, 3), strides=1, padding='same', name='conv2D_1')(X)
    # X = Conv2D(filters=64, kernel_size=(5, 5), strides=1, padding='same', name='conv2D_3')(X)
    ...
    # Block2
    X = Conv2D(filters=128, kernel_size=(3, 3), strides=1, padding='same', name='conv2D_2')(X)
    # X = Conv2D(filters=128, kernel_size=(5, 5), strides=1, padding='same', name='conv2D_5')(X)
    ...
    # Block3
    X = Conv2D(filters=256, kernel_size=(3, 3), strides=1, padding='same', name='conv2D_3')(X)
    # X = Conv2D(filters=128, kernel_size=(5, 5), strides=1, padding='same', name='conv2D_5')(X)
    ...
    X = Dropout(rate=.5)(X)
    X = Dense(units=256, activation="relu", name='fc_1')(X)
    X = Dropout(rate=.5)(X)
    X = Dense(units=2, activation="softmax", name='fc_2')(X)
